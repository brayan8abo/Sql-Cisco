...
select empleado.*, (select nombre from departamento where empleado.id_departamento = departamento.id) from empleado; -- con subconsulta
...
SELECT 
    ...
    (SELECT nombre FROM departamento WHERE departamento.id = empleado.id_departamento) AS nombre_departamento,
    departamento.*
FROM
    empleado, departamento
WHERE
    empleado.id_departamento = departamento.id
ORDER BY 
    nombre_departamento, empleado.apellido1, empleado.apellido2, empleado.nombre; -- con subconsulta
...
select distinct empleado.*,(select distinct departamento.id from departamento where empleado.id_departamento = departamento.id and departamento.nombre = 'Sistemas' or 'Contabilidad' or 'I+D') from empleado order by empleado.nombre;
