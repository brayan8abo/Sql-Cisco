...
create procedure comprar_entrada(nif varchar(9),id_cuenta int unsigned,id_butaca int unsigned,out error int)
begin
declare error int;
...
call comprar_entrada('7Q', 1, 5);
call comprar_entrada('73024558Q', 2, 3,@error);

INSERT INTO cuentas VALUES (1,50);
INSERT INTO cuentas VALUES (2,5);

delimiter $$
drop procedure if exists comprar_entrada3$$
create procedure comprar_entrada(nif VARCHAR(9), 
								id_cuenta INT UNSIGNED,
								id_butaca INT UNSIGNED,out error int)
BEGIN
	DECLARE error INT;
...
/*llamada a funcion*/
call comprar_entrada('7Q', 1, 2,@error);
