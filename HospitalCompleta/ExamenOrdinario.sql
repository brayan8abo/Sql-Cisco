...
select libros.titulo,aniopublicacion from libros where libroid < ( select aniopublicacion from libros where titulo = "Norwegian Wood");
...
select distinct libros.titulo from libros,autores where Genero = "Novela";

select libros.titulo,autores.nombre from libros,autores where Genero = "Novela"; -- saca todo duplicado...

-- 6.     Consulta para obtener los autores que escribieron libros después del año 1980:

select autores.nombre from autores where autorid in ( select autorid from escrituras where libroid in (select libroid from libros where aniopublicacion > 1980));

-- 7.     Consulta para obtener los libros escritos por autores de nacionalidad ‘Britanico’:

select libros.titulo from libros where libroid in(select libroid from escrituras where autorid in (select autorid from autores where nacionalidad = "Britanico"));
...
select libros.titulo from libros where libroid not in (select libroid from escrituras);


-- 10.   Consulta para obtener los libros que tienen más de un autor:

select titulo from libros where libroid = (select libroid from escrituras group by libroid having count(autorid)>1);
...
select libros.titulo from libros join escrituras on libros.libroid = escrituras.libroid join autores on escrituras.autorid = autores.autorid 
where autores.nombre = "Miguel de cervantes";

-- 12.   Consulta para obtener los autores del libro titulado ‘Cumbres borrascosas’

select autores.nombre from autores join escrituras on autores.autorid = escrituras.autorid join libros on escrituras.libroid = libros.libroid where libros.titulo = "Cumbres Borrascosas";
...
select autores.nombre from autores join escrituras on autores.autorid = escrituras.autorid join libros on escrituras.libroid = libros.libroid group by nombre having count(libros.libroid)>1;

-- 15.   Consulta para obtener el título y el autor de los libros en cuyo género literario contenga la palabra ‘Novela’:

select libros.titulo from libros join escrituras on libros.libroid = escrituras.libroid where genero = "Novela"; -- deberian salir dos

-- 16.   Consulta para obtener los autores que escribieron libros después del año 1980:

select autores.nombre from autores join escrituras on autores.autorid = escrituras.autorid join libros on escrituras.libroid = libros.libroid where aniopublicacion > 1980;

-- 17.   Consulta para obtener los libros escritos por autores de nacionalidad ‘Britanico’:

select libros.titulo from libros join escrituras on libros.libroid = escrituras.autorid join autores on escrituras.autorid = autores.autorid where nacionalidad = "Britanico";

-- 18.   Consulta para obtener los autores que no escribieron ningún libro:


select nombre from autores left join escrituras on autores.autorid = escrituras.autorid where escrituras.libroid is null;


-- 19.   Consulta para obtener los libros que no tienen autor registrado:

select libros.titulo from libros left join escrituras on libros.libroid = escrituras.libroid where escrituras.autorid is null;


-- 20.   Consulta para obtener los libros que tienen más de un autor:

select libros.titulo from libros join escrituras on libros.libroid = escrituras.libroid group by libros.libroid having count(escrituras.autorid)>1;
...
update libros set genero = "Ciencia Ficción"
where libroid in (select escrituras.libroid from escrituras join autores on escrituras.libroid = autores.autorid where autores.nombre = "Haruki Murakami");

-- 23.   Actualizar la nacionalidad de todos los autores que escribieron libros en un género específico:


update autores set nacionalidad = 'Cubano' where autorid in ( select escrituras.autorid from escrituras join libros on escrituras.libroid = libros.libroid where libros.genero = "Clasico");
...
update autores set nombre = 'Juan Jose' where autorid in (select autorid from (select escrituras.autorid from escrituras
 where libroid in (select libroid from libros where autorid in (select autorid from autores where nombre = "Fernandiño"))) as consulta );
