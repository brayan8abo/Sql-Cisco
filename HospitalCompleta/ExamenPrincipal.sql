...
select distinct departamento.id, departamento.nombre, departamento.presupuesto-departamento.gastos as "Presupuesto disponible" from empleado,departamento
...
select empleado.* from empleado,departamento where empleado.id_departamento = departamento.id and departamento.nombre = "Sistemas" or departamento.nombre = "Contabilidad" or departamento.nombre = "I+D" order by empleado.nombre;
...
select e.nombre from empleado e,departamento d where e.id_departamento = d.id and d.presupuesto not BETWEEN 100000 and 200000;

select empleado.nombre from empleado,departamento where empleado.id_departamento=departamento.id 
and departamento.presupuesto > 100000 and departamento.presupuesto < 200000; -- malo lo tiene entre ese valor
...
select distinct d.nombre from empleado e,departamento d where e.id_departamento = d.id and e.apellido2 is null;
...
select d.* from departamento d where d.presupuesto not BETWEEN 100000 and 200000;


/*12. Obtener los departamentos donde los gastos son mayores que el
presupuesto.*/

select d.nombre,d.presupuesto,d.gastos from departamento d where d.gastos > d.presupuesto;
...
select e.* from empleado e where e.apellido2 is null;
...
select e.nombre from empleado e where e.apellido2 in ("Diaz","Moreno");


/*17. Obtener los nombres, apellidos y nif de los empleados que trabajan
en los departamentos 2, 4 o 5.*/

select DISTINCT empleado.nombre from empleado,departamento where empleado.id_departamento = departamento.id and departamento.id = 2 or departamento.id = 4 or departamento.id =5;
...
select departamento.nombre from empleado,departamento where empleado.id_departamento = departamento.id and empleado.nombre in ("Pepe", "ruiz", "santana");
...
select empleado.nombre from empleado join departamento on empleado.id_departamento = departamento.id;


/*2. Devuelve un listado donde sólo aparezcan aquellos empleados que
no tienen ningún departamento asociado.*/

select distinct empleado.nombre from empleado join departamento on empleado.id_departamento is null;
...
select  distinct empleado.nombre,departamento.* from empleado join departamento on empleado.id_departamento = departamento.id;
...
select empleado.nombre,departamento.* from empleado join departamento on empleado.id_departamento = departamento.id order by departamento.nombre,empleado.apellido1,empleado.apellido2,empleado.nombre;
...
select distinct d.id from departamento d join empleado e on d.id = e.id_departamento;
...
select DISTINCT d.id,d.nombre,d.presupuesto-d.gastos as "Presupuesto actual" from departamento d join empleado e on d.id = e.id_departamento;

select DISTINCT d.id,d.nombre,d.presupuesto from departamento d join empleado e on d.id = e.id_departamento;


/*10. Consulta para obtener el nombre del departamento donde trabaja
el empleado que tiene el nif 38382980M*/

select d.nombre from departamento d join empleado e on d.id = e.id_departamento and e.nif = "38382980M";

/*11. Consulta para obtener el nombre del departamento donde trabaja
el empleado "Pepe Ruiz Santana".*/

select departamento.nombre from departamento join empleado on departamento.id = empleado.id_departamento and empleado.nombre in ("Pepe","Ruiz","Santana");
...
select empleado.* from empleado join departamento on empleado.id_departamento = departamento.id and departamento.nombre = "i+d" order by empleado.nombre,apellido1,apellido2;
...
select distinct empleado.* from empleado join departamento on empleado.id_departamento = departamento.id and departamento.nombre = "Sistemas" or departamento.nombre = "i+d" or departamento.nombre = "contabilidad" 
...
select empleado.nombre from empleado join departamento on empleado.id_departamento = departamento.id and presupuesto not BETWEEN 100000 and 200000; 
...
select distinct departamento.nombre from departamento join empleado on departamento.id = empleado.id_departamento and empleado.apellido2 is null;
...
update departamento set presupuesto = presupuesto * 2 where departamento.id in (select departamento.id from departamento where departamento.id in 
(select departamento.id from departamento where gastos >20000));
